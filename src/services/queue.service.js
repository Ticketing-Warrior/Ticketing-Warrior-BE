...
export async function insertQueue(nickname) {
  ...
  const prevPos = await runLuaScript(queueScript, ["ticketQueue"], ["getMyPosition", nickname]);
  ...
  // 세션 시작 시간 기록
  await runLuaScript(queueScript, [], ["setSessionStart", nickname, Date.now()]);

  // 대기열에 추가
  await runLuaScript(queueScript, ["ticketQueue"], ["addToQueue", nickname]);
  
  // 새 순번 확인
  const curPos = await runLuaScript(queueScript, ["ticketQueue"], ["getMyPosition", nickname]);
...
export async function getOutQueue(nickname) {
  ...
  await runLuaScript(queueScript, ["ticketQueue"], ["removeFromQueue", nickname]);
}
